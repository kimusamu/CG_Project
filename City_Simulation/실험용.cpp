...
GLuint VAO_Grass, VBO_NORMAL_Grass, VBO_VERTEX_Grass;
...
void initTextures() {
    ...
    Grass_Load.InitTexture("image/grass.png");
}
...
void light() {
    // 조명
    // shaderID.setVec3("light_Flag", 1.f, 1.f, 1.f);
...
void move_place() {
    ...
    shaderID.setVec3("objectColor", 0.1f, 0.1f, 0.1f);
    glm::mat4 Tx = glm::mat4(1.0f);
    glm::mat4 Rz = glm::mat4(1.0f);
    glm::mat4 TR = glm::mat4(1.0f);
    glm::mat4 model = glm::mat4(1.0f);
    ...
    //    CostBox = glm::translate(CostBox, glm::vec3(0.f, 1.f, 0.f));
...
void Cost_place() {
    ...
    //텍스처 입히기
    //cost를 문자열로 나눠.
    //반복문 돌면서 i가 같으면 해당 i숫자 텍스처 입혀 ㅠ.ㅠ.ㅠ.ㅠ.ㅠ.
