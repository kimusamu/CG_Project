...
GLvoid TimerFunc(int value)
{
    ...
    angularSpeed += 0.00001f;
    float angleInRadians = glm::radians(angularSpeed);

    if (hour == 6) {
        angleInRadians = 15;
    }

    float newX = lightPos_x * cos(angleInRadians) - lightPos_y * sin(angleInRadians);
    float newY = lightPos_x * sin(angleInRadians) + lightPos_y * cos(angleInRadians);
...
void light() {
    ...
    if (hour >= 6 && hour <= 22) {
        ...
        shaderID.setVec3("lightColor", 0.5f, 0.5f, 0.5f);
    }
